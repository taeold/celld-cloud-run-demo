...
The model is `opencode/nemotron-3.5-lightning-free`. No API key is needed.
...
Set `URL` to your celld deployment URL. Using `curl` and `jq`:

```bash
URL="https://YOUR_CELLD_DEPLOYMENT_URL"
SESSION_ID=$(curl -fsS -X POST "$URL/sessions" | jq -r '.id')

curl -fsS -X POST "$URL/sessions/$SESSION_ID/prompt" \
...
Use the same session ID to read its history and send a follow-up:

```bash
# Read the two stored messages: your prompt and the reply.
curl -fsS "$URL/sessions/$SESSION_ID" | jq '.messages | length'
# 2
...
This sample has no authentication or shell tools. Session state is persisted to the configured GCS bucket.
